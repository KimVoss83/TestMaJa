...
// NOTIFICATION BADGES
// =========================================================
const _prevCounts = { messungen: 0, leitungen: 0, hilfslinien: 0 };
function _notifyBadge(badgeId, sectionId, newCount, key) {
...
// UNDO / REDO
// =========================================================
const CANVAS_SERIAL_PROPS = ['_measureId','_isRef','_noSelect','_circlePreview','_arcPreview',
...
function lookupSensor(make, model) {
...
function calcGSD(altM, focalMM, sensorWidthMM, imgWidthPx) {
...
function calcRequiredForTarget(targetErr_m = 0.10, dist_m = 100) {
  if (!state.scale) return null;
  const gsd_m = 1 / state.scale;
  const digitErr_m = Math.sqrt(2) * gsd_m;
...
function calcFlightRecommendation(sW_mm, f_mm, imgW_px, targetErr_m = 0.10) {
...
function flightRecommendationTableHTML(sW_mm, f_mm, imgW_px, cameraName, highlightAlt = null) {
...
// ONBOARDING
// =========================================================
function showRefOnboarding(exifResult) {
  ...
  const bindStage = (n) => {
    if (n === 1) {
      if (autoScale) {
        card.querySelector('#ob-start').onclick  = () => { close(); setTool('select'); };
        card.querySelector('#ob-refine').onclick = () => goToStage(2);
      } else {
        card.querySelector('#ob-draw-ref').onclick = () => { close(); setTool('ref'); };
        card.querySelector('#ob-no-ref').onclick   = () => { close(); setTool('select'); };
      }
    } else {
      card.querySelector('#ob-draw-ref').onclick = () => { close(); setTool('ref'); };
...
const _loupe = (() => {
  ...
  function update(clientX, clientY, offsetX, offsetY) {
    if (!_active) return;
    const d = dpr();
  ...
  return { get active() { return _active; }, show, hide, update };
})();

// ── RAF-Throttle für mouse:move renderAll ─────────────────
...
function promptReference(pxLen) {
...
// PIPE TOOL
// =========================================================
const PIPE_LINE_WIDTH = 1.5;
const PIPE_OFFSET_PX = 3; // lateral offset when pipes overlap
const PIPE_SNAP_RADIUS = 40; // px radius to show distance guides
const PIPE_HANDLE_RADIUS = 4;

function handlePipeClick(p) {
...
function updatePreviewPipe(pts) {
  ...
  showPipeDistanceGuides(pts[pts.length - 1]);
  canvas.renderAll();
}
...
// =========================================================
function startPipeEdit(polyline) {
...
function updatePipeFromHandles() {
  const ep = state.editingPipe;
  if (!ep) return;
...
function insertPipeVertex(segIndex, point) {
  const ep = state.editingPipe;
  if (!ep) return;
...
function deletePipeVertex(handleIdx) {
  const ep = state.editingPipe;
...
const PIPE_REF_LINE_COLOR = '#FF9500';
const PIPE_REF_POINT_COLOR = '#FF2D55';
const PIPE_REF_GUIDE_COLOR = 'rgba(255,59,48,0.6)';

// --- Reference creation sub-modes ---
...
function promptPipeRefName(title, cb) {
  createModal(title,
...
function createPipeRefLine(p1, p2, name) {
  const id = ++pipeRefId;
...
function createPipeRefPoint(p, name) {
  const id = ++pipeRefId;
...
function togglePipeRef(refId, active) {
...
// =========================================================
function clearPipeDistanceGuides() {
  state.pipeSnapLines.forEach(o => canvas.remove(o));
  state.pipeSnapLines = [];
}

function showPipeDistanceGuides(cursorPt) {
  clearPipeDistanceGuides();
...
function computeDimLine(pipePolyline, ref, overrideT) {
...
function clearDimLinesForPipe(measureId) {
  canvas.getObjects()
    .filter(o => o._dimLinePipeId === measureId)
    .forEach(o => canvas.remove(o));
}

function renderDimLinesForPipe(measureId) {
...
// ─── Zuordnen-Modus ──────────────────────────────────────
function startAssignMode(measureId) {
...
function confirmAssignMode() {
  const measureId = state.assignModePipeId;
  endAssignMode();
  if (measureId != null) {
    renderDimLinesForPipe(measureId);
    saveSnapshot();
  }
}
...
function toggleRefAssignment(refId) {
  const meas = state.measurements.find(m => m.id === state.assignModePipeId);
  if (!meas) return;
...
function directToggleRef(measureId, refId, checked) {
  const meas = state.measurements.find(m => m.id === measureId);
  if (!meas) return;
  if (!meas.refs) meas.refs = [];
...
// Push all pipe objects behind measurement objects
function sendPipesToBack() {
...
function offsetOverlappingPipes() {
...
// GRID OVERLAY
// =========================================================
const GRID_NICE_STEPS = [0.1, 0.25, 0.5, 1, 2, 5, 10, 25, 50, 100, 200, 500, 1000];
...
function updatePipePanel() {
  ...
  Object.entries(groups).forEach(([typeKey, items]) => {
    ...
    items.forEach(m => {
      ...
      const hasRefs = m.refs && m.refs.length > 0;
...
if (_isTouchDevice) {
  _loupe.show = function() { /* noop auf Touch */ };
}
